...
def plot_fed_balance_sheet(fed_frame, aorl='Assets'):
    mindate = min(fed_frame['Date'])
    maxdate = max(fed_frame['Date'])
    fed_init = fed_frame[fed_frame['Date']==mindate]
    fed_curr = fed_frame[fed_frame['Date']==maxdate]
    dbal_frame = pd.DataFrame([fed_curr['Balance'].reset_index(drop=True) - fed_init['Balance'].reset_index(drop=True)])
    dbal_frame.columns = fed_init['Item'].values
    dbal_frame['Securities'] = dbal_frame['Securities'] + dbal_frame['Premiums'] + dbal_frame['Discounts']
    dbal_frame = dbal_frame.drop(['Premiums','Discounts'], axis=1)
    dbal_frame['Other assets'] = dbal_frame['Other 1'] + dbal_frame['Other 2'] + dbal_frame['Other 3'] + dbal_frame['Other 4'] + dbal_frame['Other 5'] + dbal_frame['Other 6'] + dbal_frame['Other 7'] + dbal_frame['Other 8'] + dbal_frame['Other 9'] + dbal_frame['CB liquidity swaps']
    dbal_frame = dbal_frame.drop(['Other 1','Other 2','Other 3','Other 4','Other 5','Other 6', 'Other 7', 'Other 8', 'Other 9', 'CB liquidity swaps'],axis=1)
    dbal_frame['Other liabilities'] = dbal_frame['Other Liability 1'] + dbal_frame['Other Liability 2'] + dbal_frame['Other Liability 3']
    dbal_frame = dbal_frame.drop(['Other Liability 1', 'Other Liability 2', 'Other Liability 3'], axis=1)

    plt.close()
    if aorl=='Assets':
        asset_frame = dbal_frame[['Other assets','Other repurchase','Liquidity facilities','Securities']].transpose()
        plt.barh(y=asset_frame.index, width = asset_frame['Balance']/1000)
        plt.title('Change in Fed assets')
    else:
        liability_frame = dbal_frame[['Other liabilities','Other deposits','Currency','Reverse repo','Bank deposits']].transpose()
        plt.barh(y=liability_frame.index, width = liability_frame['Balance']/1000)
        plt.title('Change in Fed liabilities')
    
    plt.tight_layout(pad=1.4)
    plt.xlabel('USD (Billions)')
    return plt

# changes per report https://www.federalreserve.gov/releases/h41/20230330/
def load_fed_balance_sheet(from_file=True):

    if from_file==True:
        fed_frame = pd.read_csv("fed_frame.csv", index_col=0)
    else:
        from fredapi import Fred
        fred = Fred(api_key_file='C:\\Keys\\api_key_file.txt')
        fed_series = ['WSHOSHO','WUPSHO','WUDSHO','WORAL','WLCFLL','H41RESPPAAENWW','H41RESPPAATAL2HNWW','H41RESPPAADHNWW','WOFSRBFL','WGCAL','WFCDA','WOSDRL','WOFRAL','WTCOL','SWPT','WLRRAL','WLODLL','WCICL','WOFDRBORBL','WOFDRBTHL','H41RESH4ENWW','WOLCL']
        fed_names = ['Securities','Premiums','Discounts','Other repurchase','Liquidity facilities','Other 1', 'Other 2', 'Other 3', 'Other 4', 'Other 5', 'Other 6', 'Other 7', 'Other 8', 'Other 9','CB liquidity swaps','Reverse repo','Bank deposits','Currency','Other deposits','Other Liability 1','Other Liability 2', 'Other Liability 3']
        fed_data = []

        for fed, name in zip(fed_series, fed_names):
            fed_hist = fred.get_series(fed, observation_start='2023-03-08')
            fed_data.append([fed_hist[0],name,fed_hist.index[0]])
            fed_data.append([fed_hist[-1],name,fed_hist.index[-1]])

        fed_frame = pd.DataFrame(fed_data, columns=['Balance','Item','Date'])
        fed_frame.to_csv("fed_frame.csv")
    return fed_frame
